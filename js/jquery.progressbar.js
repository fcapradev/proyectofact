...
(function($) {
	$.extend({
		progressBar: new function() {
			...
			this.defaults = {
				steps			: 5,								// steps taken to reach target
				stepDuration	: 5,										
				...
				callback		: null,								// CB with the config object that has the current percentage, target percentage, current image, etc
			...
			this.construct = function(arg1, arg2) {
				...
				return this.each(function(child) {
					...
					if (argvalue != null && this.bar != null && this.config != null) {
						...
						config.id				= $this.attr('id') ? $this.attr('id') : Math.ceil(Math.random() * 1000000);	// random id, if none provided
