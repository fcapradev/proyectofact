...
if(isset($_REQUEST['pro'])){
	...
	$num_provincia = $_REQUEST['pro'];

	$_SESSION['ParSQL'] = "SELECT ID, NOMBRE FROM ANOMPRO WHERE ID = ".$num_provincia; 
	$R1TB = mssql_query($_SESSION['ParSQL']) or die("Error SQL");
	rollback($R1TB);
	if(mssql_num_rows($R1TB) == 0){	
		?>
		<script>
			jAlert('El n&uacute;mero de Provincia no existe.', 'Debo Retail - Global Business Solution');
			document.getElementById('<? echo $se; ?>lugar').value = "";
			document.getElementById('<? echo $se; ?>tipopro').value = "< ELEGIR UNA PROVINCIA >";
		</script>    
		<?
		exit;
	}
    ...
    <script>
		...
		if(controldeins == "a"){
			...
			document.getElementById('<? echo $se; ?>tipopro').value = "<? echo $NOMBRE; ?>";
			document.getElementById('<? echo $se; ?>idtipopro').value = "<? echo $NOMBRE; ?>";
...
document.onkeydown = function(){
	...
	if(window.event){
		...
		if(window.event.keyCode == 38){
			...
			if(concontador <= 0){ 
				...
				for (i=1; i<=contador_total; i++){
					...
					if(i != contador){
						$('#<? echo $se; ?>litachep'+contador).removeClass("chelineare2").addClass("chelineare1");
					}	
			...
			if(document.getElementById('<? echo $se; ?>litachep'+contador).className == 'chelineare1'){
				$('#<? echo $se; ?>litachep'+contador).removeClass("chelineare1").addClass("chelineare2");
			}

			if(contador_cappp == 0){
				if(contador_capss > 0){
					contador_cappp = 5;
					movpag_a_prosel(contador_capss);
		...
		if(window.event.keyCode == 40){
			...
			if(contador < contador_total || contador == 0){
				...
				if(contador_cappp == 6){
					if(contador_capss != 0){
						contador_cappp = 1;
						movpag_b_prosel(contador_capss);
						contador_capss = contador_capss + 1;
					}
				}
		...
		if(document.getElementById("<? echo $se; ?>descpidetipopro").style.display == "block"){	
			if(window.event.keyCode == 13){
				if(contador != 0){
					document.getElementById('<? echo $se; ?>litachep'+contador).onclick();
					return false;
				}
			}
		}
...
function movpag_a_prosel(p){
	
	np = p - 1;
	document.getElementById("capa_prosel"+np).style.display="block";
	document.getElementById("capa_prosel"+p).style.display="none";

return false;
}

function movpag_b_prosel(p){
...
function <? echo $se; ?>envia_pro(id,des){
	
	var controldeins = "<? echo $se; ?>";
	if(controldeins == "a"){

		document.getElementById('<? echo $se; ?>tipopro').value = des;
		document.getElementById('<? echo $se; ?>idtipopro').value = id;
		document.getElementById('<? echo $se; ?>lugar').value = id;
	
		SoloNone("<? echo $se; ?>fondodescpro, <? echo $se; ?>fondodescproimg, <? echo $se; ?>descpidetipopro");
...
$_SESSION['ParSQL'] = "SELECT ID, NOMBRE FROM ANOMPRO ORDER BY ID";
$R1TB = mssql_query($_SESSION['ParSQL']) or die("Error SQL");
rollback($R1TB);
...
if($ID == -1){ $ID = 0; }
	...
	if ($c == 1){
		...
		echo "<div id=\"capa_prosel".$s."\" style=\"display:".$e."\">";
		
		if($s <> 1){

			?>
        
    	    <div id="<? echo $se; ?>Anterior_SigP" style="	position:absolute; top:-2px; left:290px;">
			<button class="StyBoton" onClick="return movpag_a_prosel(<?php echo $s; ?>)" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('Anterior_SigP<?php echo $s; ?>','','otros/scr_arri-over.png',0)"><img src="otros/scr_arri-up.png" border="0" id="<? echo $se; ?>Anterior_SigP<?php echo $s; ?>"/></button>
			</div>
	...
	?>
	<div id="<? echo $se; ?>litachep<? echo $cc; ?>" class="chelineare1" onClick="<? echo $se; ?>envia_pro(<? echo $ID; ?>,'<? echo trim($NOMBRE); ?>');">
	...
	if ($c == 5){
		?>
        <div id="<? echo $se; ?>Siguiente_SigP<?php echo $s; ?>" style="position:absolute; top:108px; left:290px;">
	        <button class="StyBoton" onClick="return  movpag_b_prosel(<?php echo $s; ?>)" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('Siguiente_SigP<?php echo $s; ?>','','otros/scr_aba-over.png',0)"><img src="otros/scr_aba-up.png" border="0" id="<? echo $se; ?>Siguiente_SigP<?php echo $s; ?>"/></button>
        </div>
        </div>
...
if ($t == 5){
	?>
	<script>
		SoloNone("<? echo $se; ?>Siguiente_SigP<?php echo $s-1; ?>");
    </script>
	<?
}
