...
<?
	...
	$_SESSION['ParSQL'] = "SELECT CANT_COMP FROM APARSIS";
	$APARSIS = mssql_query($_SESSION['ParSQL']) or die("Error SQL");
	rollback($APARSIS);		
	while ($reg=mssql_fetch_array($APARSIS)){
	
		$CANT_COMP = $reg['CANT_COMP'];
...
$SQL = "SELECT  TOP ".$CANT_COMP." * FROM AMAEFACT WHERE PLA = ".$PLA." AND TCO = 'CI' AND ANU <> 'A' ORDER BY FEC DESC";		
...
while ($reg=mssql_fetch_array($registros)){
    ...
    <table width="420" cellpadding="0" cellspacing="1">
        <tr> 
            ...
            <td width="70" height="28"><div align="center"><? echo dec($reg['TOT'],2); ?></div></td>
        </tr>
	</table>  
    </div>
