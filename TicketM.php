...
//  cuando se le hace click para que muestre el detalle
   ...
   //'---------------------------------------------------------------------------------
	...
	$_SESSION['ParSQL'] = "SELECT * FROM AMAEFACT WHERE TIP = '".$t."' AND SUC = ".$s." AND TCO = '".$c."' AND NCO = ".$n." AND LUG = ".$_SESSION['ParLUG']."";
	...
	while ($R3=mssql_fetch_array($R1TB)){
	   ...
	   //'---------------------------------------------------------------------------------
		...
		$_SESSION['ParSQL'] = "SELECT COD,NOM,DOM,LOC,PCI FROM CLIENTES WHERE COD = ".$R3['COD'];
		$R2TB = mssql_query($_SESSION['ParSQL']) or die("Error SQL");
		rollback($R2TB);		
		while ($R2=mssql_fetch_array($R2TB)){
   ...
   //'---------------------------------------------------------------------------------
	$_SESSION['ParSQL'] = "SELECT * FROM AMOVFACT WHERE TIP = '".$t."' AND TCO = '".$c."' AND SUC = ".$s." AND NCO = ".$n."";
	$R2TB = mssql_query($_SESSION['ParSQL']) or die("Error SQL");
	rollback($R2TB);		
	...
	while ($R2=mssql_fetch_array($R2TB)){
		 ...
		 $laCOD = format($R2['COD'],2,'0',STR_PAD_LEFT)."-".format($R2['ART'],4,'0',STR_PAD_LEFT);
		 $LAdet = $R2['TIO'];
		 $laUNI = dec($R2['PUN'], 2);
		...
		<table cellpadding="1" cellspacing="1">
			<tr>
				<td width="66" align="left"><? echo $laCOD; ?></td>
